...
RDF stands for **R**esource **D**escription **F**ramework which is a framework for describing resources on the web. It was initially designed to represent metadata on the Web. However, nowadays RDF is the foundational data model for [Semantic Web](https://www.w3.org/standards/semanticweb/). In addition, RDF along with other technologies such as SPARQL, OWL, and SKOS empower [Linked Data](https://www.w3.org/standards/semanticweb/data). In other words, RDF is fun, easier than relational databases and efficient to use.
...
Now that we know how things are described based on ontologies, we can look on the web to find the identifiers which are attributed to each class and individual. For our example, let's take a look at [DBpedia](https://wiki.dbpedia.org/). DBpedia is an amazing platform which provides linked data features for the content of [Wikipedia](https://www.wikipedia.org/). In other words, it is a tool to semantically query whatever exists on Wikipedia. For instance, `http://dbpedia.org/resource/France` and  `http://dbpedia.org/resource/Paris` are the identifiers for [France](http://dbpedia.org/resource/France) and [Paris](http://dbpedia.org/resource/Paris) respectively. You get something like the following for Paris as the capital of France:
...
There are various ways to represent RDF data, namely [RDF/XML](https://www.w3.org/TR/2004/NOTE-owl-parsing-20040121/), [RDFa](https://www.w3.org/TR/rdfa-lite/), [JSON-LD](https://www.w3.org/TR/2018/WD-json-ld11-20181214/), [N-Triples](https://www.w3.org/TR/n-triples/) and [Turtle](https://www.w3.org/TR/turtle/), among which we will focus on the two latter ones in this tutorial. 

#### N-Triples

N-Triples is a line-based and a concrete syntax for RDF. N-Triples are a sequence of RDF terms in the form of `subject predicate object`, separated by white space and terminated by a '.'. The following shows the abovementionend example in N-Triples:
...
- **Domain** and **Range** of properties: Think of a `property` as a function. A function has a domain and a range. The domain is where the function (or here the property) gets as input (i.e. domain) and produced the output (i.e. range).
